...
switch($_GET[act]){
	// menampilkan pengetahuan korelasi anatara gejala dan penyakit
  default:
  $offset=$_GET['offset'];
	//menampilkan jumlah data per halaman
	$limit = 15;
	if (empty ($offset)) {
		$offset = 0;
	}
  $tampil=mysqli_query($connection,"SELECT * FROM basis_pengetahuan ORDER BY kode_pengetahuan");
	echo "<form method=POST action='?module=pengetahuan' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
          <br><br><table class='table table-bordered'>
		  <tr><td><input class='btn bg-olive margin' type=button name=tambah value='Tambah Basis Pengetahuan' onclick=\"window.location.href='pengetahuan/tambahpengetahuan';\"><input type=text name='keyword' style='margin-left: 10px;' placeholder='Ketik dan tekan cari...' class='form-control' value='$_POST[keyword]' /> 
		  <input class='btn bg-olive margin' type=submit value='   Cari   ' name=Go></td> </tr>
          </table></form>";
		  	$baris=mysqli_num_rows($tampil);
	if ($_POST[Go]){
			$numrows = mysqli_num_rows(mysqli_query($connection,"SELECT * FROM basis_pengetahuan b,penyakit p where b.kode_penyakit=p.kode_penyakit AND p.nama_penyakit like '%$_POST[keyword]%'"));
			if ($numrows > 0){
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>"; 
	$hasil = mysqli_query($connection,"SELECT * FROM basis_pengetahuan b,penyakit p where b.kode_penyakit=p.kode_penyakit AND p.nama_penyakit like '%$_POST[keyword]%'");
	$no = 1;
	$counter = 1;
    while ($r=mysqli_fetch_array($hasil)){
	if ($counter % 2 == 0) $warna = "dark";
	else $warna = "light";
	$sql = mysqli_query($connection,"SELECT * FROM gejala where kode_gejala = '$r[kode_gejala]'");
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>
		  "; 
	$hasil = mysqli_query($connection,"SELECT * FROM basis_pengetahuan ORDER BY kode_pengetahuan limit $offset,$limit");
	$no = 1;
	$no = 1 + $offset;
	$counter = 1;
    while ($r=mysqli_fetch_array($hasil)){
	...
	$sql = mysqli_query($connection,"SELECT * FROM gejala where kode_gejala = '$r[kode_gejala]'");
	$rgejala=mysqli_fetch_array($sql);
	$sql2 = mysqli_query($connection,"SELECT * FROM penyakit where kode_penyakit = '$r[kode_penyakit]'");
	...
	if ($offset!=0) {
		...
		echo "<span class=disabled>Back</span>";//mencetak setiap halaman tanpa link
	}
	//menghitung seluruh jumlah halaman
	...
	//mengecek halaman yang berada di posisi akhir
  ...
  case "tambahpengetahuan":
	echo "	<div class='alert alert-success alert-dismissible'>
				...
				<b>Contoh:</b><br>
				Jika kepercayaan <b>(MB)</b> anda terhadap gejala daun menguning untuk penyakit Layu Bakteri adalah <b>0.8 (Hampir Pasti)</b><br>
				Dan ketidakpercayaan <b>(MD)</b> anda terhadap gejala daun menguning untuk penyakit Layu Bakteri adalah <b>0.2 (Hampir Mungkin)</b><br><br>
				<b>Maka:</b> CF(Pakar) = MB – MD (0.8 - 0.2) = <b>0.6</b> <br>
				Dimana nilai kepastian anda terhadap gejala daun menguning untuk penyakit Layu Bakteri adalah <b>0.6 (Kemungkinan Besar)</b>
              </div>
		...
		$hasil4 = mysqli_query($connection,"SELECT * FROM penyakit order by nama_penyakit");
		...
		$hasil4 = mysqli_query($connection,"SELECT * FROM gejala order by nama_gejala");
		while($r4=mysqli_fetch_array($hasil4)){
  ...
  case "editpengetahuan":
    $edit=mysqli_query($connection,"SELECT * FROM basis_pengetahuan WHERE kode_pengetahuan='$_GET[id]'");
