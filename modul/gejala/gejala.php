...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($connection,"SELECT * FROM gejala ORDER BY kode_gejala");
	...
	if ($_POST[Go]){
			$numrows = mysqli_num_rows(mysqli_query($connection,"SELECT * FROM gejala where nama_gejala like '%$_POST[keyword]%'"));
			if ($numrows > 0){
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>"; 
	$hasil = mysqli_query($connection,"SELECT * FROM gejala where nama_gejala like '%$_POST[keyword]%'");
	$no = 1;
	$counter = 1;
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>
		  "; 
	$hasil = mysqli_query($connection,"SELECT * FROM gejala ORDER BY kode_gejala limit $offset,$limit");
	$no = 1;
	$no = 1 + $offset;
	$counter = 1;
    while ($r=mysqli_fetch_array($hasil)){
	...
	for($i=1;$i<=$halaman;$i++){
		...
		if($offset!=$newoffset){
			...
			echo "<span class=current>".$i."</span>";//mencetak halaman tanpa link
		}
	}

	//mengecek halaman paling akhir
	if(!(($offset/$limit)+1==$halaman) && $halaman !=1){

		//jika hasil cek bukan halaman terakhir maka akan dila jutkan ke halaman berikutnya
  ...
  case "editgejala":
    $edit=mysqli_query($connection,"SELECT * FROM gejala WHERE kode_gejala='$_GET[id]'");
