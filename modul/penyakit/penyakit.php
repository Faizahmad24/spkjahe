...
if (!(isset($_SESSION['username']) && isset($_SESSION['password']))) {
  ...
  switch ($_GET[act]) {
    ...
    default:
      ...
      $tampil = mysqli_query($connection,"SELECT * FROM penyakit ORDER BY kode_penyakit");
      ...
      if ($_POST[Go]) {
        $numrows = mysqli_num_rows(mysqli_query($connection,"SELECT * FROM penyakit where nama_penyakit like '%$_POST[keyword]%'"));
        if ($numrows > 0) {
		  ...
		  <tbody>";
          $hasil = mysqli_query($connection,"SELECT * FROM penyakit where nama_penyakit like '%$_POST[keyword]%'");
          $no = 1;
          $counter = 1;
        ...
        if ($baris > 0) {
		  ...
		  <tbody>
		  ";
          $hasil = mysqli_query($connection,  "SELECT * FROM penyakit ORDER BY kode_penyakit limit $offset,$limit");
          $no = 1;
          $no = 1 + $offset;
          $counter = 1;
          while ($r = mysqli_fetch_array($hasil)) {
          ...
          //mengecek halaman pada posisi akhir
          if (!(($offset / $limit) + 1 == $halaman) && $halaman != 1) {

            //jika hasil cek bukan halaman terakhir maka akan diberikan next
    ...
    case "editpenyakit":
      $edit = mysqli_query($connection,"SELECT * FROM penyakit WHERE kode_penyakit='$_GET[id]'");
